...
use projects::PROJECTS_IN_MEMORY;
...
async fn handle_projects_message(
    msg: MethodCall,
    ...
    let mut projects = PROJECTS_IN_MEMORY.lock();
    ...
    let ranked = if query.is_empty() || force_execute {
        ...
        projects.filter_on_query(&query, cwd.clone())
    };
    let initial_size = projects.len();
    ...
    if let Some(lnum) = lnum {
        ...
        if let Some(new_entry) = ranked.get(lnum as usize - 1) {
            ...
            preview = Some(json!({ "lines": vec!["TODO: show project"], "fname": new_curline }));
        }
    }
...
impl ClapProvider for ProjectsProvider {
    ...
    async fn on_create(&mut self, call: Call) {
        let initial_lines =
            handle_projects_message(call.unwrap_method_call(), self.context.clone(), true)
                .await;
    ...
    async fn on_typed(&mut self, msg: MethodCall) -> Result<()> {
        let new_lines = tokio::spawn(handle_projects_message(
            msg,
            self.context.clone(),
            false,
        ))
        .await
        .unwrap_or_else(|e| {
            tracing::error!(error = ?e, "Failed to spawn task handle_message");
            Default::default()
        });
